...
pub fn swap_taglist_entries(
...
) -> Result<Vec<Track>, String> {
    {
        let db = state.db.lock();
        crate::tag_index::backfill_unindexed_tracks(&db)?;
        if let Ok(partner_id) = db.resolve_taglist_swap_partner(
            taglist_id,
            source_value.as_deref(),
            target_value.as_deref(),
            source_track_id,
        ) {
            let playing = state.player.state().track_id;
            if playing == Some(source_track_id) || playing == Some(partner_id) {
                state.player.stop();
            }
        } else if state.player.state().track_id == Some(source_track_id) {
            state.player.stop();
        }
    }
    ...
    let tracks = {
        let db = state.db.lock();
        crate::taglist_swap::swap_taglist_entries(
            &db,
            taglist_id,
            source_value.as_deref(),
            target_value.as_deref(),
