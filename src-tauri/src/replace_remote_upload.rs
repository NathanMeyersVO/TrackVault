...
impl ReadyNotifier {
    ...
    fn signal_ready(&self, port: u16) {
        if let Some(tx) = self.sender.lock().take() {
            let _ = tx.send(ServerStartMsg::Ready { port });
        }
    }
...
impl ReplaceRemoteUploadManager {
    ...
    fn start_with_kind(
        ...
        *self.server_thread.lock() = Some(handle);
        ...
        let port = match ready_rx.recv_timeout(SERVER_START_TIMEOUT) {
            Ok(ServerStartMsg::Ready { port }) => {
                self.log_event(
                    app_data_dir,
                    &session_log_id,
                    "INFO",
        ...
        let upload_url = upload_url_for_ip(primary_ip, port, &token);
        let localhost_test_url = upload_url_for_ip(Ipv4Addr::LOCALHOST, port, &token);
        let alternate_urls: Vec<String> = lan_ips
            .iter()
            .filter(|ip| **ip != primary_ip)
            .map(|ip| upload_url_for_ip(*ip, port, &token))
            .collect();
...
async fn run_server(
    ...
    let listener = match TcpListener::bind("0.0.0.0:0").await {
        ...
        Err(error) => {
            return fail(format!("Failed to bind upload server: {error}"));
        }
    };
    ...
    let port = match listener.local_addr() {
        Ok(addr) => addr.port(),
        Err(error) => {
            return fail(format!("Failed to read upload server port: {error}"));
        }
    };
    log_line("bind_ok", "INFO", &format!("port={port}"));
    ...
    let std_listener = match listener.into_std() {
        ...
        Err(error) => {
            log_line(
                ...
                "ERROR",
                &format!("err=\"Failed to configure upload listener: {error}\""),
            );
            return fail(format!("Failed to configure upload listener: {error}"));
        }
    };
    ...
    let app_router = Router::new()
        .route("/s/:token", get(upload_page).post(receive_upload))
    ...
    let result = tokio::select! {
        result = server_future => {
            result.map_err(|e| format!("Upload server stopped: {e}"))
        }
...
fn upload_url_for_ip(ip: Ipv4Addr, port: u16, token: &str) -> String {
    format!("https://{ip}:{port}/s/{token}")
}

fn enumerate_private_ipv4s() -> Result<Vec<Ipv4Addr>, String> {
...
fn pick_primary_ipv4(ips: &[Ipv4Addr]) -> Ipv4Addr {
    ips.iter()
